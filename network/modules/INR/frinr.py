...
class FrINR(nn.Module):
    ...
    def forward(self, x):
        ...
        # Save Dimensions
        # [1, 64, 360, 640] -> [1, 360, 640, 64] -> [1, 230400, 64]
        dims = x.shape
        x = x.permute(0, 2, 3, 1).reshape(1, -1, 64)
        if self.pe==True:
            x=self.positional_encoding(x)
        output=self.net(x)
        output=output.reshape(dims[0],dims[2],dims[3],-1).permute(0,3,1,2)
        return output

    def from_config(config):
        return FrINR(config['mode'],config['in_features'],config['hidden_features'],config['hidden_layers'],config['out_features'],config['outermost_linear'],config['high_freq_num'],config['low_freq_num'],config['phi_num'],config['alpha'],config['first_omega_0'],config['hidden_omega_0'],config['pe'])
...
if __name__ == "__main__":
    model = get_INR('sin+fr', 64 , 256, 4, 12, True, 10, 10, 4, 0.01, 30, 30, False)
    
    x = torch.randn(1, 64, 360, 640)
    print(x.shape)
    y = model(x)
    print(y.shape)
    print(y)
